...
func TestCP1_EsquemaMigra(t *testing.T) {
	...
	columnasPedido := []string{"producto_id", "cliente_id", "cantidad", "estado", "total"}
	for _, col := range columnasPedido {
		if !m.HasColumn(&models.Pedido{}, col) {
			t.Errorf("falta la columna %q en la tabla de Pedido (¿definió el campo en el modelo?)", col)
		}
	}
}
